...
What this does not do is get you a Bazel. `.bazelversion` names `buildbuddy-io/5.0.382`, and that release has darwin and linux assets and no Windows one, so bazelisk downloads nothing and stops with a 404. Pointing the same tree at a stock Bazel release builds a target, which is how the wrappers were checked. That gap is #243.
