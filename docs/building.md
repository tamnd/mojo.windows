...
What stops it after that is #253. The three cc toolchain drivers are bare Python scripts, Windows has no shebang, and `CreateProcess` will not start a `.py` whatever the file association says, so the first compile action fails to launch with error 193 until they get a launcher.
