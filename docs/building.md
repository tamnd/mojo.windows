...
What stops it after that is #260. The rc turns on `--experimental_output_paths=strip`, Bazel only implements path mapping for spawns that run sandboxed or remotely, and there is no sandbox on Windows, so every C++ compile in the tree refuses to run. Passing `--experimental_output_paths=off` clears that by hand and the build then reaches #253, where it stays. The three cc toolchain drivers are bare Python scripts, Windows has no shebang, and `CreateProcess` will not start a `.py` whatever the file association says, so the first compile action fails to launch with error 193 until they get a launcher.
