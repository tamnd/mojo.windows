...
What stops it after that is #263, the length of a command line. cmd.exe caps at 8191 characters and `CreateProcess` at 32767, and a few hundred compile actions and seven link actions in the compiler's graph are longer than that. The fix is Bazel's `compiler_param_file`, `archive_param_file` and `linker_param_file` features on the Windows toolchain, which needs `linker-driver.py` to learn to expand a params file since it reads its own arguments out of `argv`. Underneath that is #264, where the `llvm-project` repository reaches its sources through relative symlinks that do not resolve through the execroot junction on Windows, so an LLVM compile is handed a source path that is not there.
