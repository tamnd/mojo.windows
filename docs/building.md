...
So the friction list turned out to be mostly folklore, the hang that looked like Bazel's fault was ours, and the one thing still standing between a Windows host and a build is a toolchain that has never been asked to run on Windows.
