...
A Windows host now gets through analysis on the default rc with nothing on the command line, and asking it for a real target then walks it through the execution side one failure at a time. The first was #259. `builtin_module_map` wrote the crosstool module map by shelling out to `find`, and on a machine with WSL installed Bazel resolves the shell to `bash.exe` in System32 and runs the script inside the Linux distribution, where none of the Windows execroot paths exist. The list `find` was producing is one Bazel already had, since the same depset was being handed to the action as its inputs, so the rule builds the file in Starlark now and there is no action left to fail.

The second was #260 and is the same shape as #256. The rc turns on `--experimental_output_paths=strip` to stop a configuration transition from rebuilding C++ that did not change. Bazel only implements path mapping for a spawn that runs sandboxed or remotely, and there is no sandbox on Windows, so every C++ compile in the tree refused to run and blamed the strategy flags for being too strict. Nothing was too strict. The two strategies a Windows host has are the only two it can have and neither of them can do path mapping, so the requirement could never be met. The flag is now set on the `linux` and `macos` arms rather than unconditionally, which is worth knowing about if you are comparing rebuild counts across machines: a Windows host rebuilds C++ across a configuration change that would have been free on the other two. That is the right trade while the alternative is not building at all.

The third was #253. The three cc toolchain drivers are bare Python scripts, Windows has no shebang, and `CreateProcess` will not start a `.py` whatever the file association says, so the first compile action failed to launch with error 193 before anything had been compiled. Each driver now gets a small `.bat` generated beside it on a Windows host, and that is what the `cc_tool` points at, because Bazel does know how to start a `.bat` through cmd.exe. The interpreter is written into the `.bat` as an absolute path rather than looked up on `PATH`, because the rc sets `--incompatible_strict_action_env` and on Windows that gives an action a `PATH` of system32, windows, wbem and powershell and no Python installer puts anything in any of them. `bazel/internal/cc-toolchain/host_python_repository.bzl` is what resolves the path, at loading time where a repository rule is allowed to look at the machine. It tries `python`, `py` and then `python3`, runs each candidate before believing it so that the `python3.exe` App Execution Alias in WindowsApps does not get picked, and takes `MOJO_HOST_PYTHON` as an override. If it finds nothing it writes an empty path rather than failing, so that analysis on a machine with no interpreter still works, the same way it works with no sysroot.

The fourth was #263, the length of a command line. cmd.exe caps at 8191 characters and `CreateProcess` at 32767, and a `.bat` launcher goes through cmd.exe, so the lower of the two is the one that applies to a compile. Counted over the compiler's own action graph, 413 of 5442 compiles and 13 of 690 archives were past 8191, and all 16 links were, 11 of them past 32767 as well. The fix is to put the arguments in a file and hand the tool `@file` instead, which is three separate switches. `compiler_param_file` is a marker feature Bazel's compile action builder looks for, so it is declared next to the other markers and enabled on a Windows host. `linker_param_file` already exists in rules_cc with the flag that names the file attached to it, so that one is enabled by its rules_cc label rather than declared again, and declaring it twice is an analysis error rather than an override. Both of those do nothing on their own, because rules_cc checks `supports_param_files` on the toolchain before it looks at any feature and defaults it to false, so that attribute is set on a Windows host too and it is what turns on links and, with `archive_param_file`, archives. `linker-driver.py` had to learn to read a params file, since it takes its own arguments out of `argv` and would otherwise see one `@file` and none of the things it looks for, and it writes the link line back out to a file afterwards because a line that did not fit going in does not fit coming out either.

All three switches are keyed on the host and not on the target, because the question is what the machine running the action will accept, and they are off everywhere else on purpose. Nothing is broken on Linux or macOS, and turning them on there would change the command line of every compile and link in the tree, which costs everybody a full rebuild to fix a problem they do not have.

The fifth was #264, and it is about how Windows follows a symlink. `llvm-project` is not a checkout. It is a repository Bazel builds at fetch time by walking the real LLVM tree in `llvm-raw` and linking every file and directory into place, which is what `configure.bzl` in the LLVM Bazel overlay does. Those links are made with `repository_ctx.symlink`, which writes an absolute target, and then Bazel rewrites every link in a fetched repository whose target is under the output base into a relative one so that the repository can be moved. On Linux that is invisible, because a relative link resolves against the directory the link is really in whichever path was used to reach it. Windows resolves it against the path that was used to reach the link instead, and an action reaches an external repository through a junction Bazel plants per repository in the execroot. Bazel only plants one for a repository the build actually uses, and nothing depends on `llvm-raw` directly, so `../../+llvm_source+llvm-raw/llvm/lib` came out somewhere under the execroot where there is no such directory and every LLVM source file was missing as far as a compile action was concerned.
