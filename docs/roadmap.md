...
**The elaborator might need to JIT target code.** Monomorphisation runs code at compile time, through either an interpreter or the JIT. If evaluating for a Windows target requires JIT running target code on the host, then the COFF JIT moves from M6 to M3 and becomes critical path, and the cross compile first strategy partly collapses. This is currently an inference, not something verified in source. It is the single assumption whose failure most reorders the plan, so check it in M1. Reading the interpreter against JIT selection is about a day's work and de-risks the whole schedule.
