...
// Everything from here to logHostMachineInfo is POSIX.  siginfo_t, sigaction,
// the si_code constants, SIGBUS, SIGSYS and alarm are all things Windows does
// not have, and there is no small edit that gives them to it.  What Windows
// gets instead is the LLVM half of this file, which is where the stack trace
// comes from and is the part anybody actually reads.  The detail block above
// it, the one naming the signal and the faulting address, is missing there.
// Getting it back means reading an EXCEPTION_RECORD out of a vectored
// exception handler, which is a different piece of work and not this one.
#ifndef _WIN32
...
static void developmentSignalHandler(void *context) {
  ...
  // Print Python stack trace if available using async-safe approach.  Windows
  // has no SIGUSR2 for faulthandler to be waiting on, so there is nothing to
  // raise and enablePythonStackTraceCallback can never make this true there.
#ifndef _WIN32
...
void M::Init::registerDevelopmentSignalHandler(llvm::StringRef programName) {
  ...
  llvm::call_once(flag, [&]() {
    ...
    // handlers.  On Windows there is nothing to install, and the LLVM handler
    // registered above is the whole of what this function does.
#ifndef _WIN32
    struct sigaction sa;
