...
struct OperandNeedingOrigin {
  ...
  /// The underlying type is spelled out because without it the two ABIs pick
  /// different ones.  An enum with no fixed underlying type gets whichever type
  /// the implementation likes that fits, and the Itanium ABI picks an unsigned
  /// one here while the Microsoft ABI picks int, which cannot hold this value
  /// at all.  Clang says so with -Wmicrosoft-enum-value and then carries on
  /// with -2, and the first place that lands in a braced initializer for a
  /// size_t is a hard error.  Every reader of this constant treats it as an
  /// operand index, so size_t is also what it should have said in the first
  /// place.
  enum : size_t {
