...
ObjectCompiler::create(StringRef basePath, CompilationOptions options,
  ...
  // lld is one binary with three drivers in it, and the name it is invoked
  // under picks which one runs. This only matters when `lld_path` is unset and
  // the linker has to come off PATH, because otherwise `-flavor` decides.
  StringRef linkerFileName = "ld.lld";
  switch (llvm::Triple(options.targetTriple).getObjectFormat()) {
  case llvm::Triple::MachO:
    linkerFileName = "ld64.lld";
    break;
  case llvm::Triple::COFF:
    linkerFileName = "lld-link";
    break;
  default:
    break;
  }
...
createSharedObject(BufferRef buf, CompilationOptions options,
  ...
  // These describe the target, not the machine doing the linking, so they come
  // off the triple. The names are only used for temporary files, but lld-link
  // reads the extension of its output to decide whether it is writing an image
  // or something else, so `.dll` here is load bearing.
  llvm::StringRef libInExt = isCOFF ? ".obj" : ".o";
  llvm::StringRef libOutExt = isCOFF ? ".dll" : ".so";
  ...
  // COFF spells the output the way link.exe does, and the string has to outlive
  ...
  // For COFF (Windows)
  //  lld-link /DLL /NOENTRY tmp.obj /out:tmp.dll
  SmallVector<StringRef> lldArgs = [&]() -> SmallVector<StringRef> {
    if (isCOFF) {
      // /DLL is the COFF -shared and /NOENTRY says there is no DllMain here,
      // which there is not because this object comes straight out of codegen.
      //
      // There is deliberately no counterpart to the Mach-O `-undefined
      // dynamic_lookup` and no equivalent of leaving ELF symbols to be bound at
...
ErrorOrSuccess ObjectCompiler::emitSharedObject(OwningOpRef<ModuleOp> module,
  ...
  // ELF, MachO and COFF object formats are supported for shared-object
  // emission. Generalize to other platforms+formats when needed.
  if (!llvm::is_contained(
          {llvm::Triple::ELF, llvm::Triple::MachO, llvm::Triple::COFF},
          triple.getObjectFormat()))
    return Error("cannot create shared object binary from target triple that "
                 "is not ELF, MachO or COFF");
