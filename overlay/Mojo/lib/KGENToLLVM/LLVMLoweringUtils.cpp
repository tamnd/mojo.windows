...
Value VariantHelper::materializeLLVMUnion(
  ...
  // The position is spelled as an ArrayRef rather than left to the implicit
  // conversion because a bare zero of any integer type is ambiguous here when
  // the compiler is built for Windows.  InsertValueOp::create is overloaded on
  // ArrayRef<int64_t> and on DenseI64ArrayAttr, and mlir::Attribute has an
  // implicit constructor from a storage pointer.  Under -fms-compatibility a
  // constant expression that evaluates to zero is still a null pointer
  // constant, which is the pre C++11 rule that MSVC never dropped, so the
  // attribute overload becomes viable and neither candidate wins.  Any nonzero
  // position is fine, which is why only the zeroes in this file and the two
  // next to it ever failed.
...
static Value lowerStringToGlobalConstant(StringAttr strAttr,
  ...
  // ArrayRef rather than a bare zero, for the reason written out above.
  Value structVal0 = LLVM::InsertValueOp::create(
      b, b.getLoc(), undefOp, llvmString, ArrayRef<int64_t>{0});
