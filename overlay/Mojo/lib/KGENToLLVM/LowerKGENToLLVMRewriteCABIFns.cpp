...
static SmallVector<Value> reconstructCABIArguments(
  ...
  for (auto [coercion, origArg] : llvm::zip(argInfos, origArgs)) {
    ...
    if (coercion.isIdentity()) {
      ...
      // ArrayRef rather than a bare zero.  A zero of any integer type is still
      // a null pointer constant under -fms-compatibility, which makes the
      // DenseI64ArrayAttr overload of create viable and the call ambiguous.
      // The long version of this is in LLVMLoweringUtils.cpp.  The line below
      // is fine as it stands because one is not zero.
      pair = InsertValueOp::create(b, pair, arg1, ArrayRef<int64_t>{0});
