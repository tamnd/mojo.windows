...
// The two worklists below are std::deque and nothing here included <deque>.
// libstdc++ and libc++ both drag it in behind something else, and Microsoft's
// STL does not, so this only ever showed up when the compiler was built for a
// Windows target.
#include <deque>
#include <filesystem>
...
static ASTDecl *buildModuleOrPackageDecl(const std::filesystem::path &path,
  ...
  if (path.extension() == ".mojo") {
    ...
    std::string fullPath;
    // AddIncludeFile takes a std::string. std::filesystem::path converts to one
    // implicitly only where its value_type is char, which is everywhere except
    // Windows, where it is wchar_t.
