...
#ifdef _WIN32
// The only thing this file wanted out of <unistd.h> is write(), and the CRT
// spells that _write() and declares it in <io.h>.  The two differ in the type
// of the count argument, unsigned int rather than size_t, which does not
// matter at the one call site because it has already narrowed the count to an
// int by then.
#include <io.h>
#else
#include <unistd.h>
...
#ifdef _WIN32
  int numWritten = _write(static_cast<int>(fdOr.get()), (const void *)*mem,
