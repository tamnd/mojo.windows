...
The C `unsigned long` type is typically a 64-bit integer on macOS and Linux, and
a 32-bit integer on Windows."""
...
def _c_long_dtype[unsigned: Bool = False]() -> DType:
    ...
    comptime if is_64bit() and (
        ...
        # LLP64: Windows leaves long at 32 bits on 64-bit systems and gives the
        # extra width to `long long` and to pointers instead. This is the one
        # place the three data models we support disagree, so anything that
        # wants 64 bits from a C header has to say `long long` and not `long`.
