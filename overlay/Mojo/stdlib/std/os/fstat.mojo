...
def lstat[PathLike: stdPathLike](path: PathLike) raises -> stat_result:
    ...
    On Windows the C runtime has no `lstat` and no flag that turns its `stat`
    into one, so this is assembled out of Win32 calls and gets two of the three
    things right. Whether the name is a link, and the answer for a link whose
    target no longer exists, are both correct. The reported size of a link is
    zero rather than the length of the target string, which is what Windows
    reports and there is no cheap way to the other number.
    """
    var fspath = path.__fspath__()
