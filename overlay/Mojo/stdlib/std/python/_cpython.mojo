...
struct CPython(Defaultable, Movable):
    ...
    def __init__(out self):
        ...
        try:
            ...
            # An empty name is not a name, and asking the loader to open it is
            # a different thing on different systems. On Linux it comes back as
            # the current process again, which is the handle already in hand,
            # so nothing happens and the missing library is reported through
            # `init_error` below, which is the intended path. On Windows
            # `LoadLibraryExW` fails on it, so the same missing library ends
            # the program here with an abort naming a path that is blank. Not
            # asking is the same answer on both.
            if not self.lib.check_symbol("Py_Initialize") and python_lib:
