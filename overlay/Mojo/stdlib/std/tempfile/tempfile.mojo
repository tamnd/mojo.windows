...
def _try_to_create_file(dir: StringSlice) -> Bool:
    for _ in range(TMP_MAX):
        ...
        #
        # Both arms set `writable` and the return is below the handler rather
        # than inside the two arms, so that `filename` has a use after the
        # handler. Its last use is otherwise an argument of a call that raises,
        # which destroys it on the way out of that call, and naming it inside
        # the handler does not count as a use. The cleanup would then be
        # looking at a slot something else has taken: on Linux the old bytes
        # are usually still there and it works by luck, on Windows the
        # allocator scrubs them, `exists` says no and the temporary file is
        # left behind. See #207.
        var writable: Bool
        try:
