...
# ===-----------------------------------------------------------------------===#
#
# Windows has all four of the clocks above and shares none of the names, so this
# is a lookup table rather than a port. The `_CLOCK_` values it switches on are
# the Darwin numbers, which is an accident of how the constants above are
# written, and it does not matter: nothing outside this file passes a clock id
# in, so all they have to be is distinct.
#
# Two things are worth knowing before reading it.
#
# The units are not nanoseconds anywhere. Windows counts in hundred nanosecond
# intervals almost everywhere it reports a time, and the performance counter
# counts in its own ticks whose rate you have to ask for. Neither is a constant
# you can bake in.
#
# The epoch is not 1970. A FILETIME counts from the first of January 1601, which
# is the start of the four hundred year Gregorian cycle that contained the
# design meeting, and the offset to the Unix epoch is a fixed number of hundred
# nanosecond ticks.

comptime _FILETIME_TICKS_PER_NSEC = 100
"""A FILETIME tick is a hundred nanoseconds."""

comptime _FILETIME_TICKS_TO_UNIX_EPOCH = 116_444_736_000_000_000
"""FILETIME ticks between the first of January 1601 and the first of January
1970, which is three hundred and sixty nine years with eighty nine leap days in
them."""


def _win_query_performance_counter() -> Int64:
    var ticks = Int64(0)
    _ = external_call["QueryPerformanceCounter", Int32](Pointer(to=ticks))
    return ticks


def _win_query_performance_frequency() -> Int64:
    var hertz = Int64(0)
    _ = external_call["QueryPerformanceFrequency", Int32](Pointer(to=hertz))
    return hertz


def _win_monotonic_nanoseconds() -> Int:
    var hertz = _win_query_performance_frequency()
    if hertz <= 0:
        # Documented as impossible since Windows XP. Returning zero rather than
        # dividing by it, because a clock that reads zero is obvious and a
        # crash inside a timer is not.
        return 0
    var ticks = _win_query_performance_counter()

    # Divide first. The counter runs at ten megahertz on most machines and
    # counts from boot, so multiplying by a billion before dividing overflows a
    # signed sixty four bit value after about fifteen minutes of uptime. Whole
    # seconds first and then the remainder keeps both halves small, and the
    # remainder is below the frequency by construction so it cannot overflow
    # either.
    var seconds = ticks // hertz
    var remainder = ticks % hertz
    return Int(seconds * 1_000_000_000 + (remainder * 1_000_000_000) // hertz)


def _win_realtime_nanoseconds() -> Int:
    # The precise variant, which reads the same clock the ordinary one does but
    # without rounding the answer down to the system timer interval. The
    # ordinary GetSystemTimeAsFileTime is accurate to about fifteen
    # milliseconds, which would make a realtime clock in nanoseconds a
    # misleading thing to offer.
    var ticks = Int64(0)
    external_call["GetSystemTimePreciseAsFileTime", NoneType](Pointer(to=ticks))
    return Int(
        (ticks - _FILETIME_TICKS_TO_UNIX_EPOCH) * _FILETIME_TICKS_PER_NSEC
    )


def _win_cputime_nanoseconds[thread: Bool]() -> Int:
    # Four FILETIMEs out, of which the first two are when the process or thread
    # started and stopped and the last two are how much of the CPU it used.
    # Only the last two are wanted here, and they are wanted added together,
    # because the POSIX CPU time clocks count kernel and user time as one
    # number.
    ...
    comptime if thread:
        var handle = external_call[
            "GetCurrentThread", OpaquePointer[MutUntrackedOrigin]
        ]()
        ...
        var handle = external_call[
            "GetCurrentProcess", OpaquePointer[MutUntrackedOrigin]
        ]()
    ...
    return Int((kernel + user) * _FILETIME_TICKS_PER_NSEC)


def _win_gettime_as_nsec(clockid: Int) -> Int:
    if clockid == _CLOCK_REALTIME:
        return _win_realtime_nanoseconds()
    if clockid == _CLOCK_PROCESS_CPUTIME_ID:
        return _win_cputime_nanoseconds[thread=False]()
    if clockid == _CLOCK_THREAD_CPUTIME_ID:
        return _win_cputime_nanoseconds[thread=True]()

    # Both monotonic clocks, and they are the same clock. The raw variant on
    # Linux means the hardware counter with no adjustment from the time
    # synchronisation daemon applied to it, and the performance counter is
    # already that: nothing on Windows slews it, and the only thing that can
    # change its rate is the machine going to sleep.
    return _win_monotonic_nanoseconds()
...
def _realtime_nanoseconds() -> Int:
    ...
    return _gettime_as_nsec(_CLOCK_REALTIME)
...
def _monotonic_nanoseconds() -> Int:
    ...
    comptime if is_gpu():
        ...
        return _gettime_as_nsec(_CLOCK_MONOTONIC)


@always_inline
def _monotonic_raw_nanoseconds() -> Int:
    """Returns the current monotonic time in nanoseconds"""
    return _gettime_as_nsec(_CLOCK_MONOTONIC_RAW)
...
def _process_cputime_nanoseconds() -> Int:
    ...
    return _gettime_as_nsec(_CLOCK_PROCESS_CPUTIME_ID)
...
def _thread_cputime_nanoseconds() -> Int:
    ...
    return _gettime_as_nsec(_CLOCK_THREAD_CPUTIME_ID)
...
def sleep(sec: Float64):
    ...
    comptime if CompilationTarget.is_windows():
        return _win_sleep_nanoseconds(Int(sec * NANOSECONDS_IN_SECOND))
...
def sleep(sec: Int):
    ...
    comptime if CompilationTarget.is_windows():
        return _win_sleep_nanoseconds(sec * _NSEC_PER_SEC)
...
comptime _CREATE_WAITABLE_TIMER_HIGH_RESOLUTION = 0x2
comptime _TIMER_ALL_ACCESS = 0x1F0003
comptime _INFINITE = 0xFFFFFFFF


def _win_sleep_nanoseconds(nanoseconds: Int):
    """Sleeps for at least the given number of nanoseconds.

    Args:
        nanoseconds: How long to sleep for. Values <= 0 return immediately.
    """
    if nanoseconds <= 0:
        return

    # `Sleep` is the obvious call and it is not good enough on its own. It takes
    # milliseconds, so it cannot express most of what this function is asked
    # for, and it rounds up to the system timer interval, which is fifteen
    # milliseconds by default. A caller asking for a microsecond would get
    # fifteen thousand of them and no indication that anything had happened.
    #
    # A high resolution waitable timer is the call that does what the signature
    # says. It has been available since Windows 10 version 1803, and on anything
    # older the create fails and there is nothing better than `Sleep` to fall
    # back to, so that is what happens.
    var timer = external_call[
        "CreateWaitableTimerExW", OptionalPointer[NoneType, MutUntrackedOrigin]
    ](
        # No security attributes and no name, so the timer is private to this
        # process and nothing else can open it.
        OptionalPointer[NoneType, ImmUntrackedOrigin](),
        OptionalPointer[UInt16, ImmUntrackedOrigin](),
        UInt32(_CREATE_WAITABLE_TIMER_HIGH_RESOLUTION),
        UInt32(_TIMER_ALL_ACCESS),
    )
    if not timer:
        return _win_sleep_milliseconds(nanoseconds)

    # Negative means relative, in the same hundred nanosecond ticks everything
    # else on this platform counts in. Positive would be an absolute time, which
    # is a different and much more surprising thing to ask for by accident.
    var due = -Int64((nanoseconds + 99) // 100)
    var armed = external_call["SetWaitableTimer", Int32](
        timer.value(),
        Pointer(to=due),
        # No period, so it fires once. No completion routine and no argument for
        # one, and no waking the machine from sleep to deliver it.
        Int32(0),
        OptionalPointer[NoneType, ImmUntrackedOrigin](),
...
def _win_sleep_milliseconds(nanoseconds: Int):
    # Rounding up, because the contract is to sleep for at least as long as
    # asked, and rounding down would turn any sub millisecond sleep into no
    # sleep at all.
    external_call["Sleep", NoneType](
        UInt32((nanoseconds + _NSEC_PER_MSEC - 1) // _NSEC_PER_MSEC)
    )
