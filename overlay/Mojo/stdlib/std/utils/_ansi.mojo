...
from std.sys._win import console_takes_escapes
from std.sys.info import CompilationTarget


def _is_stdout_tty() -> Bool:
    comptime if CompilationTarget.is_windows():
        # Being a terminal is not the question on Windows. `isatty` says yes to
        # a pipe and to the null device, and a console that is one says no to
        # escape sequences until it has been asked for them. One call answers
        # both, because it is the call that does the asking.
        return console_takes_escapes()
