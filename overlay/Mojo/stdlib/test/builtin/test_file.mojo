...
def test_file_binary_round_trip() raises:
    """Whatever bytes go into a file are the bytes that come back out.

    This is the test that fails when a Windows port forgets `O_BINARY`, and it
    is worth having as a test rather than a comment because the failure has no
    other symptom: text mode is not an error, nothing reports it, and the only
    way to notice is that the file does not match itself.

    Three separate things go wrong without the flag and all three are covered
    here. Writing turns every newline into a carriage return and a newline, so
    the file grows. Reading turns every carriage return and newline back into a
    newline, so a file that genuinely contained both loses one. And a byte 26,
    which is Ctrl-Z, ends a text mode read where it sits, so everything after
    it disappears.
    """
    var temp_file = Path(gettempdir().value()) / "test_file_binary_round_trip"
    ...
    # Every byte value, so that 10, 13 and 26 are all in there, and then the
    # two byte sequences on their own so that a translation in either direction
    # changes the length.
    var content = List[Byte](capacity=256 + 8)
    for value in range(256):
        content.append(Byte(value))
    # A carriage return and newline pair, the same pair reversed, a Ctrl-Z with
    # something after it, and a trailing pair.
    var tail: List[Int] = [13, 10, 10, 13, 26, 65, 13, 10]
    for value in tail:
        content.append(Byte(value))

    with open(temp_file, "w") as f:
        f.write_bytes(Span(content))

    with open(temp_file, "r") as f:
        var read_back = f.read_bytes()
        assert_equal(
            len(read_back),
            len(content),
            "file changed length on the way through",
        )
        for index in range(len(content)):
            assert_equal(
                Int(read_back[index]),
                Int(content[index]),
                String("byte ", index, " changed on the way through"),
            )
...
def test_file_open_fifo() raises:
    ...
    comptime if CompilationTarget.is_windows():
        # No FIFOs and no mkfifo. Windows named pipes are a different thing
        # with a different API, and nothing in `FileHandle` reaches them, so
        # there is no Windows shaped version of this test to write.
        return
