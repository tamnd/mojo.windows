...
# The Microsoft runtime's `strerror` knows the classic codes and stops at
# EILSEQ, so the messages for everything POSIX added later come from the table
# in `sys._libc_errno` instead. Both halves are checked here, and the numbers
# these names carry are not the Linux ones: ETXTBSY is 139 rather than 26 and
# EWOULDBLOCK is 140 rather than the same number as EAGAIN.
comptime error_message_windows: List[Tuple[ErrNo, String]] = [
    ...
    (ErrNo.EOWNERDEAD, "Owner died"),
    (ErrNo.ENOTRECOVERABLE, "State not recoverable"),
    (ErrNo.STRUNCATE, "Output was truncated"),
    (ErrNo.EOTHER, "Other error"),
]
