...
///   - `-l<name>` or `-l <name>`: locate `lib<name>.so` (Linux),
///     `lib<name>.dylib` (macOS) or `<name>.dll` (Windows) within the search
///     path. macOS also accepts `lib<name>.so`, because plugins there are
///     often built that way. Windows has no `lib<name>.dll` form, which is a
///     name nothing on that system is actually called.
...
///     (`.so`, `.dylib`, or a versioned `.so.N` form, and `.dll` on
///     Windows): loaded directly. The set is the one this platform's loader
///     accepts rather than all of them, since the file is about to be loaded
///     into this process.
///
