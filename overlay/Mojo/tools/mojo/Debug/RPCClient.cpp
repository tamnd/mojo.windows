...
static ErrorOrSuccess invokeRPC(bool dryRun, ArrayRef<int> ports,
  ...
  llvm::raw_fd_ostream extraLogStream(logFileOrErr->getFD(),
                                      /*shouldClose=*/true,
                                      /*unbuffered=*/true);
  // raw_ostream has no operator<< for std::filesystem::path either. It picks
  // the StringRef one through the implicit conversion to std::string, and that
  // conversion is not there on Windows, where path::value_type is wchar_t.
