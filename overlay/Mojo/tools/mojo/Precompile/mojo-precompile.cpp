...
static ErrorOr<bool> isExistingDirectory(const std::filesystem::path &path) {
  std::error_code ec;
  // formatv has no provider for std::filesystem::path. It reaches the one for
  // std::string through the implicit conversion, which exists only where
  // path::value_type is char. On Windows it is wchar_t, so say string().
