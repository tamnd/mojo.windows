...
static int run(const State &subcommandState) {
  ...
  if (ErrorOr<MojoConfig> configOr = MojoConfig::open(); !configOr.isError()) {
    ...
    // The host namer, and deliberately so. `mojo build` asks its target triple
    // this question because what comes back is linked into somebody else's
    // binary, but a JIT'd program is loaded into this process, so here the
    // machine that matters is the one running the compiler.
    configOr->appendSharedLibraryLinkArgs(
        configuredLibs, PlatformLibrary::getSharedLibraryName);
