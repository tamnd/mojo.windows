...
def _process_cc_dep_list(deps):
    ...
    return new_deps, needs_wheel

def _process_cc_deps(data, deps):
    # This used to iterate deps directly, with a TODO saying it would break on a
    # select(), and it did: passing one got "type 'select' is not iterable" and the
    # whole package failed to load. Decomposing first is what _process_defines above
    # already does for exactly the same reason, so this is the same shape.
    #
    # Deps that differ by platform are unavoidable once a third target platform is in
    # play, because a dependency that has no arm for that platform has to be droppable
    # rather than merely swappable. The first case here was crashpad, which the BUILD
    # file we fetch wires up for Linux and macOS only.
    new_deps = []
    needs_wheel = False
    for is_select, elements in decompose_select_elements(deps):
        if is_select:
            new_select = {}
            for key, values in elements.items():
                mapped, wheel = _process_cc_dep_list(values)
                new_select[key] = mapped
                needs_wheel = needs_wheel or wheel
            new_deps += select(new_select)
        else:
            mapped, wheel = _process_cc_dep_list(elements)
            new_deps += mapped
            needs_wheel = needs_wheel or wheel

    # If any arm of a select pulls in max, the wheel goes in data for every arm. data
    # could be made to select the same way, but the wheel is a runfile rather than a
    # dependency, so the cost of over-approximating is a file that does not get read,
    # and the cost of getting it wrong the other way is a missing runfile at test time.
    return {
        "deps": new_deps,
