...
load(":cross_platform_test.bzl", "HOST_PLATFORM_INPUTS", "HOST_PLATFORM_OUTPUTS", "host_platform_settings")
...
# A lit test is a host test under every configuration. It drives the mojo driver at test
# time and that driver targets the machine it runs on, so nothing in one of these belongs
# on a Windows box, and the platform half below says so. See cross_platform_test.bzl for
# the rest of the reasoning. It is merged into the strip transition rather than sitting
# beside it because a rule gets one incoming transition.
def _test_transition_impl(settings, attr):
    output = host_platform_settings(settings)
...
_test_transition = transition(
    ...
    inputs = HOST_PLATFORM_INPUTS + [_STRIP_OPTION],
    outputs = HOST_PLATFORM_OUTPUTS + [_STRIP_OPTION],
)
...
EOF
    ...
    # The rule above rather than upstream's, so that this one gets the host transition too.
    # It reads the test sources and nothing else, so it was never a target platform test.
    py_test(
