...
def _shared_library_impl(ctx):
    ...
    # `dynamic_library` is normally a symlink in the _solib_<cpu> tree and
    # `resolved_symlink_dynamic_library` is the file it points at, which is the one
    # everything below wants. For a Windows target there is no symlink and no
    # _solib tree: the copy_dynamic_libraries_to_binary feature is on, and Bazel
    # skips the symlink when it is, because it puts a copy of the library beside
    # each binary that needs it instead. `dynamic_library` is then the real file
    # already and the resolved field is None.
    ...
    dwarf_outputs.append(dynamic_library)
    ...
    stripped_output = ctx.actions.declare_file(
        ctx.label.name + ".stripped/" + dynamic_library.basename,
    )
    register_strip_action(
        ctx = ctx,
        input_file = dynamic_library,
