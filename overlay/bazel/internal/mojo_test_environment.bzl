    ...
    for lib in cc_deps:
        ...
        for linker_input in lib[CcInfo].linking_context.linker_inputs.to_list():
            for library in linker_input.libraries:
                # A static library has no dynamic_library, and everything below
                # is about staging a shared object next to the test and pointing
                # an rpath at it, which a static one does not need. Upstream has
                # no static implicit dep so this never came up. The Windows
                # toolchain has two, the compiler-rt builtins and the maths
                # replacements, and without this the rule dies on a None with a
                # traceback that names neither. The loop below over the deps
                # reached through Mojo already skips them the same way.
