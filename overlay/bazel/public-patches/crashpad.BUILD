...
cc_binary(
    ...
    srcs = [
        ...
        # Empty on purpose, and not because nobody got round to writing it.
        # Upstream crashpad has a handler/win, so a real third arm could be filled
        # in here. We decided not to: CrashReporting already has a Windows arm with
        # no crashpad in it, which is #79, and the Windows crash handler is going to
        # be a vectored exception handler instead, which is #25. Building a handler
        # we have decided never to run is work for nothing.
        #
        # The target_compatible_with below is what makes the empty list safe. Without
        # it this would be a cc_binary with no platform sources and it would fail at
        # link time with a missing main, which is a much worse error to hand somebody
        # than the name of the target.
        #
        # Both are needed. target_compatible_with is read during rule analysis, and
        # selects on attributes are resolved before that, so marking the target
        # incompatible on its own does not stop this select from failing to match.
